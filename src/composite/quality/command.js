...
export async function handleQualityCommand(
    ...
    try {
        ...
        spinner.text = chalk.cyan(
            `Running Quality composite graph via ${activeProviderName}...`
        );
        ...
        spinner.succeed(
            chalk.green(
                `Quality report written to ${globalConfig.outputDir || 'Review'}/QUALITY_REPORT.md`
            )
        );
        ...
        spinner.fail(
            chalk.red(
                `Quality routine aborted: ${error.message}`
            )
        );
