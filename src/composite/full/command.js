...
export async function handleFullCommand(
    filePath,
    providerName,
    modelName
) {
    ...
    const spinner = ora(
        'Indexing target repository layout...'
    ).start();
    ...
    try {
        ...
        spinner.text = chalk.cyan(
            `Running Full composite graph via ${activeProviderName}...`
        );
        ...
        spinner.succeed(
            chalk.green(
                `Full report written to ${globalConfig.outputDir || 'Review'}/FULL_REPORT.md`
            )
        );
        ...
        spinner.fail(
            chalk.red(
                `Full routine aborted: ${error.message}`
            )
        );
