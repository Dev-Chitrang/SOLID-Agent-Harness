...
export async function handleFullCommand(filePath, providerName, modelName) {
    ...
    try {
        ...
        spinner.text = chalk.cyan(`Running Full composite graph via ${activeProviderName}...`);
        ...
        spinner.succeed(chalk.green(`Full analysis complete. Underlying reports updated in ${globalConfig.outputDir || 'Review'}/\n`));

        const table = new Table({
            head: [chalk.green('Full Analysis Summary')],
            wordWrap: true,
            colWidths: [80]
        });
        table.push([summary]);
        ...
        spinner.fail(chalk.red(`Full routine aborted: ${error.message}`));
    }
}
