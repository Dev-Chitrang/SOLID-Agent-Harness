...
export async function handleOnboardCommand(filePath, providerName, modelName) {
    ...
    try {
        ...
        spinner.text = chalk.cyan(`Running Onboarding composite graph via ${activeProviderName}...`);
        ...
        spinner.succeed(chalk.green(`Onboarding complete. Underlying reports updated in ${globalConfig.outputDir || 'Review'}/\n`));

        const table = new Table({
            head: [chalk.magenta('Onboarding Summary')],
            wordWrap: true,
            colWidths: [80]
        });
        table.push([summary]);
        ...
        spinner.fail(chalk.red(`Onboarding routine aborted: ${error.message}`));
    }
}
