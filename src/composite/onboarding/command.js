...
export async function handleOnboardCommand(
    ...
    try {
        ...
        spinner.text = chalk.cyan(
            `Running Onboard composite graph via ${activeProviderName}...`
        );
        ...
        spinner.succeed(
            chalk.green(
                `Onboard report written to ${globalConfig.outputDir || 'Review'}/ONBOARD_REPORT.md`
            )
        );
        ...
        spinner.fail(
            chalk.red(
                `Onboard routine aborted: ${error.message}`
            )
        );
